...
To ensure maximum transparency in ALL hands played, several novel smart contract have developed to manage ALL critical gameplay elements on-chain.

#### There are currently three (3) smart contracts that manage Cast Poker's platform:
...
Manages an eternal database, which supports a global storage for ALL contract data.
